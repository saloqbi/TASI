...
function Get-FreshQuote {
  ...
  $active=''; try {$active=[string]$script:ab.ActiveDocument.Name} catch {}
...
try {
  ...
  Write-Host "V502 AMIBROKER RECOVERY LINK READY: http://127.0.0.1:$Port/quote" -ForegroundColor Green
  Write-Host "Reads XAUUSD directly after RefreshAll; active chart may be any symbol. Keep this window open." -ForegroundColor Cyan
  while($true){
    ...
    try {
      ...
      if($req.method-eq'GET'-and $path-eq'/health'){
        Reply $stream 200 'OK' ([ordered]@{ok=(Ensure-Ab);service='amibroker-recovery-v502';symbol=$fixedSymbol;port=$Port;lastPrice=$lastPrice;error=$lastError}|ConvertTo-Json -Compress);continue
      }
