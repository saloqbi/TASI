...
function Get-GoldQuote {
  ...
  if ($null -eq $script:stock) {
    $script:stock = $script:ab.Stocks.Item($script:fixedSymbol)
  }
  if ($null -eq $script:stock) { throw "Symbol '$($script:fixedSymbol)' was not found in AmiBroker." }
  $quotes = $script:stock.Quotations
...
try {
  ...
  $listener.Start()
  Write-Host "GOLD FAST bridge LIVE on http://127.0.0.1:$Port/quote" -ForegroundColor Green
  ...
  while ($true) {
    ...
    try {
      ...
      if ($request.method -eq 'GET' -and $path -eq '/health') {
        $payload = [ordered]@{ok=(Ensure-AmiBroker);service='amibroker-gold-v498-fast';symbol=$fixedSymbol;lastPrice=$script:lastPrice;lastDirection=$script:lastDirection;error=$script:lastError;port=$Port} | ConvertTo-Json -Compress
